...
class DialogSynchroAruco(QDialog):
    """
    Dialog for VIS / NIR timeline synchronization and pairing using ArUco-based angle estimation.
    ...
    Phase 3 of the processing pipeline:
      ...
      - 3.3 pairing VIS and NIR images
    """
    ...
    def _init_gui(self) -> None:
        ...
        # Boutons Run ArUco / Compute time alignment / appariement
        ...
        self.btn_image_pairing = QPushButton("Pair VIS / NIR images")
        self.btn_image_pairing.setEnabled(False)
        btn_layout.addWidget(self.btn_image_pairing)
    ...
    def _connect_signals(self) -> None:
        ...
        self.btn_image_pairing.clicked.connect(self.on_image_pairing)
    ...
    def on_run_aruco(self) -> None:
        ...
        try:
            ...
            self._log(f"Starting ArUco detection (VIS + NIR) using {max_workers} threads ")
    ...
    def on_compute_time_shift(self):
        """
         Step 3.2 – Run compute time shift between VIS time line and NIR time line
        """
    ...
    def on_image_pairing(self):
        """
        Phase 3.3 – Pair VIS / NIR images using time shift
        """
        # Stop phase 3.2 animation if still running
        self.stop_timeshift_animation()
        self.timeshift_plot_label.setText("Image pairing in progress…")
        ...
        # Signals
        self.image_pairing_worker.signals.progress.connect(self.progress_bar.setValue)
        self.image_pairing_worker.signals.stage.connect(self.progress_label.setText)
        self.image_pairing_worker.signals.finished.connect(self._on_image_pairing_finished)
        self.image_pairing_worker.signals.error.connect(self._on_image_pairing_error)
        # Start
        QThreadPool.globalInstance().start(self.image_pairing_worker)

    def _on_image_pairing_finished(self, pairing_result: dict):
        self._log(f"Image pairing completed ({len(pairing_result.get('shoot_points', []))+1} pairs).")
        self.timeshift_plot_label.setText("Image pairing completed.")
        self.progress_label.setText("Image pairing completed.")
        self.progress_bar.setValue(100)

    def _on_image_pairing_error(self, message: str):
        self._log(f"Image pairing error: {message}")
        QMessageBox.critical(
            self,
            "Image pairing error",
            message
    ...
    def stop_timeshift_animation(self):
        """
        Stop animation of time-shift plots (phase 3.2).
        """
